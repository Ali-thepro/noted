...
#### **Authentication**
- Users can log in using Google and GitHub OAuth for secure and convenient access. Additionally, users have the option to register and log in with their email and password, with credentials securely stored in MongoDB. JWT are implemented to maintain secure sessions across both terminal and web platforms.

#### **Note Creation and Management**
- The ability to create, edit, and delete Markdown notes through either the terminal client or the web interface. Regardless of the preferred environement the notes will be managed seamlessly 

#### **Markdown Rendering and Preview**
- The system renders Markdown to display well-formatted notes in both terminal and web clients. Web client will display live preview, providing immediate visual feedback. For elements that cannot be rendered in the terminal, the system generates a link to view the full preview within the web client.
#### **Tagging and Organization**
- Notes can be categorized using tags, making it easy to retrieve and organize them.
...
- The system will allow for notes to be edited simultaneously in web clients. The system will manage notes using CRDTs to ensure seamless real-time synchronization and data consistency. Additionally, encrypted symmetric keys are employed to maintain data integrity and privacy throughout the collaboration process.
...
#### **Export Notes**
- The system allows users to export notes in various formats such as PDF, DOCX, and HTML. 
...
#### **Task Management**
- Basic task management features, such as to-do lists and task assignments, are integrated within notes. This allows users to organize and track tasks directly within their note-taking environment.
...
Listed below are the different scenarios users might find themselves doing.
...
The user will then create a new account if the dont have one already.
...
The user selects a note they wish to delete. The user confirms the deletion. The system removes the note from the user's account and updates the database. The user is notified if the deletion is successful. In the CLI the user enters the coommand to select the note they wish to delete.
...
The user adds or removes tags that categorize the note (e.g., “Work”, “Personal”, etc.).
The system updates the tags, and the user can filter notes by tags for better organization.

++Scenario 8++: **View Markdown Preview**
The user creates or edits a note in Markdown format. The user can click the "Preview" button to see a live preview of how the note will look once rendered. The system displays the rendered content in real-time (on the web client). In the CLI the user can run the command to show the rendered command in the terminal or get a link to the web client that will show elements that cannot be rendered in the terminal.

++Scenario 9++: **Create and Manage Tags for Notes**
The user selects a note to which they want to add tags. The user adds or removes tags that categorize the note (e.g., “Work”, “Personal”, etc.). The system updates the tags, and the user can filter notes by tags for better organization.


++Scenario 10++: **Adding a Colaborator**
n the page to edit a note, the user clicks the "Add Collaborator" button and enters the email address or username of the collaborator they want to invite. The system checks the collaborator's credentials, then sends an invitation via an email. The collaborator received the invitation and accpets it.

++Scenario 11++: **Edit a Note with a Collaborator**
The user and the collaborator open the shared note simultaneously on the web interface. Each user makes changes to the note’s content in real-time. The system synchronizes the changes, ensuring that all edits are reflected instantly across all active sessions. Conflict-free Replicated Data Types (CRDTs) automatically resolve any potential conflicts, maintaining a consistent view of the note for both users. The version history is updated to include the latest changes made by both collaborators

++Scenario 10++: **Removing a Colaborator**
...
The CLI will only be available on Unix based operating systems. It will not be available on Windows unless through WSL. The web interface must perform consistently across multiple browsers. Encryption needs to be implemented properly so notes can be encrypted and decrpyred regardless of platform.

**Performance and Speed Requirements**
By implemnting encryption and real-time collaboration, performance might be affected. We must deliver real time updates without noticeable latency to ensure user experience stays smooth.
...
## 3.1 User Authentication
 - **Description**: The system should allow users to be able to authenticate through Google OAuth, GitHub OAuth, or by creating a local account. For a local account the user must register before logging in. For OAuth the system stores the user information in the database during their first authentication. On subsequent logins, the system grants access without additional registration. Once authenticated, the user is granted access. For the CLI the system will create a directory that will store all the notes that the use will create.
 ...
 - **Dependecies with other requirements**: 
...
- **Description**: The system must implement assymetric end-to-end encryption to protect the security of user notes. Using a pair of cryptographic keys (public and private), notes should be encrypted on the user's device before being stored on the server, ensuring that only the user can access the content with their private key. For collaboration, notes are encrypted using a symmetric key which is encrypted individually using the public key of each collaborator. 
- **Criticality**: High – Ensures that the user's notes remains confidential and secure, addressing privacy concerns and protecting against unauthorised access.
...
- **Technical Issues**:
    ...
    - Integrating encryption properly on both web interface and CLI, so encryption and decrpytion is seamless across both platforms.
...
- **Technical Issues**:
    - Implementing real-time synchronization using Y.js to handle concurrent edits.
    - Ensuring only users that have been given access can edit it.
    - Ensure encryption doesn't delay transmission of data between clients. 
    - Making sure conflicts are resolved correctly.

- **Dependencies with Other Requirements**:
    - Relies on real-time Websocket communication.
    - Depends on End-to-End Encryption (3.2) for encrpyted collaboration.
    - Relies Version Control (3.10) for storing versions.
    - Depends on Add a Collaborator (3.11).
...
- **Technical Issues**: 
    - Creating a note on CLI with a command must properly store it in the database, ensuring its accessible on both clients.
    - Making sure that the direcory that stores all notes has not been deleted.
- **Dependecies with other requirements**: 
    - Relies on version control (3.9).

## 3.5 Edit Notes
- **Description**: Users must be able to edit existing notes. Users should be able to modify the content of their notes, with changes being encrypted and saved in the database with proper version tracking. Users on the web interface must also have the choice to see a live preview of the content rendered in Markdown. Users should be able to see the cursor of another user during collaboration and the changes that they have done. On the CLI this will open the note in the default editor of the terminal.
...
- **Technical Issues**:
    -  Make sure the note on the CLI has not been deleted from the main directory, system will have to grab the latest version in the database if this is the case.
    -  Managing and storing edited notes with proper version control.
    -  Ensure Markdown content is rendered correctly on both platforms.
    -  Ensuring content is synchronised during real-time collaboration.
    -  Showing changes done by different users and showing where they are in the editor.
...
- **Dependecies with other requirements**:  
    ...
    - Integrates with Real-Time Collaboration (3.3) and Image Handling with Amazon S3 (3.13).
...
- **Criticality**: High – Fundamental for users to review their notes and ensure its formatted correctly.
- **Technical Issues**: 
...
## 3.7 Push Notes
- **Description**: The system must allow the CLI client to push local changes to the server. This is to ensure edits made on terminal are sent to the database so the two clients will be kept in sync. Content of the note must be encrypted before being sent to the server.
...
- Not critical to main functionality of the system, allows users to have full control over their notes, allowing them to remove unnecessary notes.
- **Technical Issues**: 
    - Decrypting each note will take time especially if there are a lot of notes. A proper system should be developed to ensure notes that have been edited since the last pull will be gottten from the database.
- **Dependecies with other requirements**: 
    - Dependent on End-to-End Encryption (3.2) for decrypting content.
    - The backend and CLI's abilities on quickly retrieving and storing files from the database.

## 3.9 Managing Different Versions
- **Description**: The system must provide a built-in versioning system for notes, enabling users to view and restore previous versions of a note. Notes are encrypted until the user wishes to view the content where it is then decrypted.
- **Criticality**: High – Version control is important for users who need to track changes, undo mistakes, or restore previous work.
- **Technical Issues**: 
    - Main issue is to store multiple versions of notes in MongoDB efficiently, and ensuring retrieval is fast and handling rollback to previous versions in a user-friendly manner.
...
- **Technical Issues**: 
    - Must ensure symmetric key for collaboration is encrypted with the collaborators public key.
- **Dependecies with other requirements**: 
...
- **Criticality**: Medium – Not critical to main functionality of the system, allows users to have full control over their notes, allowing them to remove unnecessary notes.
- **Technical Issues**: 
...
- **Technical Issues**: 
    - Integraton of Amazon S3 with backend.
    - Reading images on the CLI and web interface so they have the correct format that is required to upload it to S3.
...
- **Technical Issues**: 
    - As notes are encrypted, searching must be done on the client. Efficeient search indexing system must be implemented to quickly search the files.
- **Dependecies with other requirements**:
    -  Integrates with Tag Management (3.14).

## 3.14 Tag Management
- **Description**: Users must be able to tag their notes with labels for categorization and easy searching. They must also be able to remove any tags as well.
- **Criticality**: Medium – Helps in organizing notes but is not critical for basic functionality.
...
- **Technical Issues**: 
    - Must ensure note is saved correctly in the CLI - to the current directory or a specified one if provided.
- **Dependecies with other requirements**: N/A
...
## 4.1.3 Collabaritive Editor
Handles the core functionality of real-time collaboration. Uses Y.js and CRDT to handle conflicts during collabration. Speaks to the websocket and ensures changes are synchronised across different clients.

## 4.1.4
The API layer provides the interface for all communication between the Frontend and the Backend.

## 4.1.5 Web Socket
Connections are made to maintain real-time sync across web clients.
...
## 4.2 Stretch Goal System Architechture
![](https://i.imgur.com/59f6p4Z.png)


## 4.2.1 Text Editor
Handles real-time collaboration. Speaks to the websocket and ensures changes are synchronised across different clients (web and CLI).
